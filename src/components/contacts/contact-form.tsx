...
export function ContactForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : 'Falha ao salvar o contato';
      toast.error(message);
    } finally {
      setSaving(false);
    }
      ...
      <DialogContent className="bg-popover border-border text-popover-foreground sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="text-popover-foreground">
            {isEdit ? 'Editar Contato' : 'Cadastrar Contato'}
          </DialogTitle>
          <DialogDescription className="text-muted-foreground">
            {isEdit
              ? 'Atualize os detalhes do contato abaixo.'
              : 'Preencha os detalhes para cadastrar um novo contato.'}
          </DialogDescription>
        </DialogHeader>
                ...
                <div className="space-y-1">
                  <p>
                    {dupMatch.exact
                      ...
                      : 'Um contato com um número muito semelhante já existe.'}
                  </p>
                  {onViewExisting && (
                    <button
                      type="button"
