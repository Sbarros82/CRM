    ...
    <TooltipProvider>
      <div className="grid grid-cols-2 gap-3 rounded-xl border border-border bg-card/60 p-4 sm:grid-cols-3 xl:grid-cols-6">
        <Metric
          ...
          tooltip="Quantidade de todos os negócios neste pipeline que não estão marcados como Perdidos. Negócios Ganhos ainda estão incluídos."
        />
        <Metric
        ...
        <Metric
          ...
          tooltip="Valor do Pipeline dividido pelo Total de Negócios — o valor médio de um negócio único não perdido."
        />
        <Metric
          icon={<TrendingUp className="h-4 w-4 text-purple-400" />}
          label="Valor Ponderado"
          value={formatCurrency(stats.weightedValue, defaultCurrency)}
          tooltip="Receita estimada: valor de cada negócio aberto × probabilidade de seu estágio. Primeiro estágio ≈ 10%, estágios progridem até 90%, Ganho = 100%. Negócios Perdidos são excluídos."
        />
        <Metric
        ...
        <Tooltip>
          <TooltipTrigger
            render={
              <button
                type="button"
                aria-label={`Como o ${label} é calculado`}
