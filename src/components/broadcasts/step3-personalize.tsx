...
const MEDIA_HEADER_LABELS: Record<MediaHeaderType, string> = {
  image: 'imagem',
  video: 'vídeo',
  document: 'documento',
};
    ...
    <div className="space-y-6">
        ...
        <div className="rounded-xl border border-border bg-card/50 p-4">
          <div className="mb-3 flex items-center gap-2">
            ...
            <span className="inline-flex items-center rounded-md bg-primary/10 px-2 py-0.5 text-xs font-medium uppercase text-primary">
              {MEDIA_HEADER_LABELS[mediaHeaderType]}
            </span>
          </div>
          ...
          <p className="mt-1.5 text-xs text-muted-foreground">
            URL pública do {mediaHeaderType === 'image' ? 'arquivo de imagem' : mediaHeaderType === 'video' ? 'vídeo' : 'documento'} enviado no cabeçalho.
      ...
      {unmappedKeys.length > 0 && (
        <div className="rounded-md border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs text-amber-300">
          Associe todos os placeholders antes de continuar — ainda faltam{' '}
          <span className="font-mono font-semibold">
            {unmappedKeys.join(', ')}
          </span>
          . Sem isso, a Meta recebe essas variáveis vazias.
        </div>
      )}
