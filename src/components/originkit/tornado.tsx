...
// Originkit "Tornado" (variant 2), adapted to the Snap palette.
// Source: https://www.originkit.dev/components/tornado?preset=variant-2
// A WebGL particle vortex: glowing strands, flickering dots and comets
// racing along the form. Respects prefers-reduced-motion and degrades
// silently when WebGL is unavailable.
...
`
  ...
  function syncColors(cfg: Config) {
  ...
  function build() {
    ...
    // Warm, neutral halo so the comet tint (brand yellow) reads cleanly.
    cometTex = track(
      blob(32, [
        [0, 'rgba(255,255,255,0.9)'],
        [0.3, 'rgba(255,240,180,0.4)'],
        [0.7, 'rgba(255,221,0,0.08)'],
        [1, 'rgba(0,0,0,0)'],
      ])
    );
  ...
  function waveAt(
    ...
    for (let i = 0; i < MAX_WAVES; i++) {
      ...
      const push =
        RIPPLE_STRENGTH * w.amp * WAVE_STRENGTH * shell * fade * near;
      ox += (dx / d) * push;
      oy += (dy / d) * push;
      oz += (dz / d) * push;
    }
  ...
  return {
    ...
    dispose() {
      ...
      for (const d of disposables) d.dispose();
      // No forceContextLoss(): React StrictMode remounts on the same canvas
      // and a lost context makes the second WebGLRenderer init fail.
      renderer.dispose();
    },
  };
}
...
export default function Tornado(props: TornadoProps) {
  ...
  React.useEffect(() => {
    ...
    try {
      ...
      console.warn('[Tornado] init failed:', err);
      return;
    }
    return () => {
